...
const OUTPUT = join(dirname(fileURLToPath(import.meta.url)), '../public/data/npm-downloads.json')
...
async function fetchMonthlyDownloads(pkg, startDate, endDate) {
  ...
  const { downloads } = await fetchDownloadRange(pkg, formatDate(startDate), formatDate(endDate))

  const downloadsByMonth = new Map()
  for (const { day, downloads: dailyCount } of downloads) {
    const yearMonth = day.slice(0, 7)
    downloadsByMonth.set(yearMonth, (downloadsByMonth.get(yearMonth) ?? 0) + dailyCount)
  }
...
async function main() {
  ...
  const packageDownloads = await Promise.all(
    PACKAGES.map(async pkg => [pkg, await fetchMonthlyDownloads(pkg, startDate, endDate)])
  )

  outputFileSync(OUTPUT, JSON.stringify(Object.fromEntries(packageDownloads), null, 2) + '\n')
